...
data = np.loadtxt('JSH_WFIRST_SNLS.HOSTLIB', skiprows=1400, usecols=columns)
...
# only use CIDint = 1
bad_cidint = np.asarray([x == 4 for x in cidint])
...
plt.hist(data_host_logmass, 40, normed=True, alpha=0.5, color='red')
plt.xlabel('host mass')
plt.xlim(7, 13.5)
plt.show()
