...
os.chdir('/Users/jaredhand/WFIRST_research/SN_distributions/host_sims/versions/GRID/hostlib/SNLS/')
columns = tuple(list(range(1, 8)))
data = np.loadtxt('JSH_WFIRST_SNLS_GRID_G10.HOSTLIB', skiprows=1010, usecols=columns)
...
plt.scatter(host_logmass, x1, alpha=0.6, label='before SNANA')
# plt.scatter(simhost_logmass, simx1, alpha=0.7, marker='x', color='red', label='after SNANA')
plt.xlabel('host mass')
plt.ylabel('x1')
plt.legend()
plt.show()

plt.scatter(host_logmass, c, alpha=0.6, label='before sim')
plt.xlabel('host mass')
plt.ylabel('c')
plt.legend()
plt.show()
