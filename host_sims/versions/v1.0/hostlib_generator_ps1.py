...
# only use CIDint = 15 for ps1
bad_cidint = np.asarray([x == 15 for x in datacidint])
...
casymgauss = asymgauss1d(cp2)
x1asymgauss = asymgauss1d(x1p2)
...
zsample = (max(datazcmb) + 0.09) * np.random.random(size=sample) + min(datazcmb) + 0.005
