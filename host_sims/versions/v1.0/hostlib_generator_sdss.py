...
def weightmaker3D(arr2, arr3, bounds, datamap, simmap, wmap0=False, step1=1., step2=1., step3=1.):
            ...
            if np.sum(wmap[i, :, j]) != 0.:
                ...
                for k in range(sample):
                    if bins2[i] <= arr2[k] < bins2[i] + step2 and bins3[j] <= arr3[k] < bins3[j] + step3 and massarr[k] == 0:
                        ...
                        # elif 10 >= mass > 9.5:
                        #     mass -= 0.5
                        while 10. + 2 * wgtstep >= mass >= 10. - 2 * wgtstep:
    ...
    for l in range(sample):
        if massarr[l] == 0.:
            ...
            while mass < 7. or mass > 13. or 10 + 2 * wgtstep >= mass >= 10 - 2 * wgtstep:
                mass = 2. / 3. * np.random.randn() + np.nanmean(datahostmass)
            massarr[l] = mass
            badcount += 1
    print(badcount)
...
# find mass asym params
# mp = mass_params(datahostmass, 10, zeroed=False)

# asym gauss dists for mass, c and x1, respectively
# massasymguass = asymgauss1d(mp)
casymgauss = asymgauss1d(cp1)
x1asymgauss = asymgauss1d(x1p1)
...
zsample = 0.49 * np.random.random(size=sample) + 0.01
masssample = 10. * np.random.random(size=sample) + 5.
...
s = [0.5, 1., 0.1]
# massx1cmap = digitize3D(masssample, x1sample, csample, massx1cbounds, step3=0.1)
